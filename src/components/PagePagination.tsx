...
const PagePagination = ({ pagination }: Props) => {
  ...
  const previus = 'flex items-center justify-center px-3 h-8 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white'
  ...
  const next = 'flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-r-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white'
    ...
    <>
      <nav aria-label="Page navigation example" className='text-center'>
        <ul className="inline-flex -space-x-px text-sm">
          <li>
            <Link href={page === 1 ? `/blog?page=${page}` : `/blog?page=${page - 1}`} className={`${previus} ${page === 1 ? 'opacity-50 pointer-events-none' : ''}`}>Previous</Link>
          </li>
